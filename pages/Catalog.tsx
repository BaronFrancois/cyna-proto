...
export const Catalog: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState(categoryFilter === 'All' ? 'Tous' : categoryFilter);
  ...
  return (
    <div className="pt-10 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-16">
        ...
        <p className="text-xl text-gray-500">Explorez notre suite d'outils de cybersécurité nouvelle génération.</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 flex flex-col group">
               ...
               <div className="p-8 flex-grow flex flex-col">
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between">
                     <div>
                        ...
                        <span className="text-gray-400 text-xs"> / {product.period === 'monthly' ? 'mois' : 'an'}</span>
                     </div>
                     <Link to={`/product/${product.id}`}>
                        <Button size="sm" variant="secondary" className="rounded-full px-5">Acheter</Button>
                     </Link>
                  </div>
               </div>
            </div>
